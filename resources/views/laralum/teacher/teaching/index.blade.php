...
@section('breadcrumb')
    <div class="ui breadcrumb">
        ...
        <i class="right angle icon divider"></i>
        <div class="active section">Teaching List</div>
    </div>
@endsection
@section('title', 'Teaching List')
@section('icon', "list")
@section('subtitle', $teacher->name)

@section('createButton')
    <a href="{{route('Laralum::teacher::teaching::create', ['teacher' => $teacher->id])}}" class='large ui green right floated button white-text'>Add Teacher Teaching</a>
@endsection
        ...
        <div class="column">
            <form>
                <div class="ui fluid action input">
                    <input type="text" placeholder="Search by course code, title, semester..."  name="search" value="{{ Request::get('search') }}">
            ...
            <a class="float-right" href="{{route('Laralum::teacher::teaching::list', ['teacher' => $teacher->id])}}">Clear Search</a>
        </div>
            ...
            <div class="ui very padded segment">
                ...
                <table class="ui selectable striped celled small table">
                    ...
                    <tr>
                        ...
                        <th class="three wide">Course Code</th>
                        <th class="five wide">Course Title</th>
                    ...
                    @foreach($educations as $key => $education)
                        <tr data-item="{{$education->id}}">
                            ...
                            <td> {{ $education->course_code }} </td>
                            <td>{!! $education->course_title !!}</td>
                            <td>{!! $education->semester !!}</td>
                            <td>
                                <a href="{{ route('Laralum::teacher::teaching::edit', ['teacher' => $teacher->id, 'teaching' => $education->id]) }}" class="mini ui blue icon button"><i class="edit icon"></i> Edit</a>
                                <a href="{{ route('Laralum::teacher::teaching::delete', ['teacher' => $teacher->id, 'teaching' => $education->id]) }}" class="delete mini ui red icon button"><i class="delete icon"></i> Delete</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
