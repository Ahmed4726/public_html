...
@section('breadcrumb')
    <div class="ui breadcrumb">
        ...
        <div class="active section">Facility Edit</div>
    </div>
@endsection
@section('title', 'Facility Edit')
@section('icon', "edit")
@section('subtitle', $facility->name)
@section('content')
    ...
    <div class="ui doubling stackable grid container">
        <div class="row">
            ...
            <div class="twelve wide column" id="vue-app">
                <form class="ui form" method="POST" action="{{ route('Laralum::facility::update', ['facility' => $facility->id]) }}" enctype="multipart/form-data">
                    <div class="ui very padded segment">
                        {{ csrf_field() }}

                        <div class="field required">
                            <label>Name</label>
                            <input type="text"  name="name" placeholder="Name..." value="{{$facility->name}}" required>
                        </div>

                        <div class="field">
                            <label>External Link</label>
                            <input type="text"  name="external_link" placeholder="External Link..." value="{{$facility->external_link}}" >
                        </div>

                        <div class="field">
                            <label>Description</label>
                            <textarea name="description" class="tinymce" id="description">{{$facility->description}}</textarea>
                        </div>

                        <div class="field">
                            <label>Contact</label>
                            <textarea name="contact" class="tinymce" id="contact">{{$facility->contact}}</textarea>
                        </div>

                        <div class="inline field">
                            <div class="ui slider checkbox">
                                <input type="checkbox" name="enabled" tabindex="0" class="hidden" @if($facility->enabled) checked @endif >
                                <label>Enabled</label>
                            </div>
                        </div>
                        ...
                        <div class="field">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="path" readonly style="border-radius: 0" value="{{$facility->image_url}}" >
                            </div>
                            <br/>
                            <img id="img-upload" @if($facility->image_url) src="{{ asset("storage/image/facility/$facility->image_url") }}" @else  src="{{ asset('storage/image/no-image.png') }}" @endif />
                        </div>--}}
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        new Vue({
            el : "#vue-app",
            data : {
                applicable : @if($facility->department_id) 'department' @elseif($facility->center_id) 'center' @else 'global' @endif,
                departments : {},
                faculty_id : "",
            ...
            methods : {
                ...
                applicableChange (event) {
                    this.applicable = event.target.value;
                }
            },
