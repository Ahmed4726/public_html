...
@section('breadcrumb')
    <div class="ui breadcrumb">
        ...
        <i class="right angle icon divider"></i><a class="section" href="{{ route("Laralum::$uri::advance", [$uri => $uriValue]) }}">Advanced Option</a>
        <i class="right angle icon divider"></i>@endif
        <div class="active section">Facilities List</div>
    </div>
@endsection
@section('title', 'Facilities List')
@section('icon', "list")
@section('subtitle', 'Facilities List')

@section('createButton')
    @if(isset($uri)) <a href="{{route("Laralum::$uri::facility::create", [$uri => $uriValue])}}" class='large ui green right floated button white-text'>Create Facilities</a>
    @else <a href="{{route('Laralum::facility::create')}}" class='large ui green right floated button white-text'>Create Facilities</a> @endif
@endsection
...
@section('content')
    ...
    <div class="ui one column doubling stackable grid container" id="vue-app" >
        ...
        <div class="column">
            <form>
                <div class="ui fluid action input">
                    ...
                    <select class="ui selection dropdown" name="status">
                        ...
                        <option @if(Request::get('status') == 'disable') selected @endif value="disable">Disable</option>
                    </select>
            ...
            @if(isset($uri)) <a class="float-right" href="{{route("Laralum::$uri::facility::list", [$uri => $uriValue])}}">Clear Search</a>
            @else <a class="float-right" href="{{route('Laralum::facility::list')}}">Clear Search</a>@endif
        </div>
            ...
            <div class="ui very padded segment">
                ...
                <table class="ui selectable striped celled small table">
                    ...
                    @foreach($facilities as $key => $facility)
                        <tr data-item="{{$facility->id}}">
                            @if($sortable)<td>{{$key+ $facilities->firstItem()}}</td>@endif
                            <td>
                                @if(!$facility->enabled)
                            ...
                            <td>
                                @if($facility->department_id && $facility->department()->exists())
                                    {{ $facility->department->short_name }}
                                    <span class="ui mini label">
                                        Department
                                    </span>
                                @elseif($facility->center_id && $facility->center()->exists())
                            ...
                            <td>
                                <a href="{{ route('Laralum::facility::edit', ['facility' => $facility->id]) }}" class="mini ui blue icon button ">
                                    <i class="edit icon"></i> Edit
                                </a>
                                <a href="{{ route('Laralum::facility::delete', ['facility' => $facility->id]) }}" class="delete mini ui red icon button ">
                ...
                $( "#sortable" ).sortable( {
                    update: function( event, ui ) {
                        $(this).children().each(function(index) {
                            $(this).find('td').first().html(index + {{$facilities->firstItem()}})
                        });
                    }
                });
                ...
                $("#applyReOrder").click(function(){
                    ...
                    axios.get("{{route('Laralum::setting::reorder')}}", {
                        params: {
                            ...
                            model : 'App\\Facility',
                            field : 'sorting_order',
                            orderStart : {{$facilities->firstItem()}}
                        }
                    }).
                    then(response => {
