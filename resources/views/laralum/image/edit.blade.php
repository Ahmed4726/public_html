...
@section('breadcrumb')
    <div class="ui breadcrumb">
        @if(isset($uri)) <a class="section" href="{{ route("Laralum::$uri::gallery::image::list", [$uri => $uriValue]) }}">Program / Calendar List</a>
        @else <a class="section" href="{{ route('Laralum::gallery::image::list') }}">Gallery Image List</a> @endif
        <i class="right angle icon divider"></i>
        <div class="active section">Image Update</div>
    </div>
@endsection
@section('title', 'Image Update')
@section('icon', "edit")
{{--@section('subtitle', $image->title)--}}
@section('subtitle', 'Image Update')
@section('content')
    ...
    <div class="ui doubling stackable grid container">
        <div class="row">
            ...
            <div class="twelve wide column" id="vue-app">
                <form class="ui form" method="POST" action="{{ route('Laralum::gallery::image::update', ['image' => $image->id]) }}" enctype="multipart/form-data">
                        <div class="ui very padded segment">
                            ...
                            <div class="field">
                                ...
                                <input type="text"  name="external_link" placeholder="External Link" value="{{ $image->external_link }}">
                            </div>
                            ...
                            <div class="field required">
                                ...
                                <select name="category_id" @change="categoryChange($event)" required>
                                    @foreach($categories as $category)
                                        <option @if($category->id == $image->category_id) selected="selected" @endif value="{{ $category->id }}">{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="inline field">
                                <div class="ui slider checkbox">
                                    <input type="checkbox" name="enabled" tabindex="0" class="hidden" style="z-index: 2" @if($image->enabled) checcked="checked" @endif>
                                    <label>Enable</label>
                                </div>
                            </div>
                            ...
                            <div class="field">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" name="path" readonly style="border-radius: 0" value="{{ $image->path }}">
                                </div>
                                <br/>
                                <img id="img-upload" @if($image->path) src='{{asset("storage/image/gallery/$image->path")}}' @else src="{{ asset('storage/image/no-image.png') }}" @endif />
                            </div>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        new Vue({
            ...
            mounted() {
                this.getSpecificInfoFromCategoryId({{$image->category_id}});
            }
