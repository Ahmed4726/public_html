    ...
    <div class="content-section people-page">
        ...
        <div class="row profile">
            ...
            <div class="col-md-3 col-sm-12 image">
                <div class="card">
                    ...
                    <img class="card-img-top img-fluid rounded-circle mx-auto d-block lazy"
                        data-src="{{asset("storage/image/administration/$member->image_url")}}" alt="{{$member->name}}">

                    <div class="card-body">
                        <h6>
                            <a href="{{$member->website_link}}">{{$member->name}}</a>
            ...
            <div class="col-md-9 col-sm-12">
                @if($member->message)<h6>{{$member->config['message'] ?? 'Message'}}</h6>{!! $member->message !!}@endif
                @if($member->address)<h6>{{$member->config['address'] ?? 'Address'}}</h6>{!! $member->address !!}@endif
            </div>
