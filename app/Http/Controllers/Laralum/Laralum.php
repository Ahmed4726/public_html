...
{
    public static function responseProgramUrl($program)
    {
        if ($program->external_url) {
            return $program->external_url;
        } elseif ($program->department()->exists() && $program->department->faculty) {
            if ($program->department->faculty->type == 'FACULTY') {
                return route('Frontend::department::program::view', ['department' => $program->department->slug, 'program' => ($program->slug) ? $program->slug : $program->id]);
            } elseif ($program->department->faculty->type == 'INSTITUTE') {
                return route('Frontend::institute::program::view', ['institute' => $program->department->slug, 'program' => ($program->slug) ? $program->slug : $program->id]);
            }
        }
    ...
    /**
     * Get User Name Before @ from Email
     *
     * @param [type] $email
     * @return void
     */
    ...
    /**
     * Parse a set of line to response array
     */
    public static function responseEveryNewLineFromText($text)
    {
        return preg_split("/((\r?\n)|(\r\n?))/", $text);
    }

    /**
     * Split config to key => value
     */
    public static function splitKeyValue($line)
    {
        if ($line = trim($line)) {
            $split = array_filter(explode('=', trim($line)));
    ...
    /**
     * Response formatted array with config
     *
     * @param [type] $text
     * @return void
     */
    public static function configTextToArray($text)
    {
        $config = [];
        foreach (static::responseEveryNewLineFromText($text) as $line) {
            $arrayData = static::splitKeyValue($line);
            if ($arrayData) {
                $config[key($arrayData)] = current($arrayData);
            }
        }
    ...
    /**
     * Format as config from array
     */
    public static function arrayToConfigFormat(array $array)
    {
        array_walk(
            $array,
            function (&$v, $k) {
    ...
    public static function generalRoles($notChangeAbleRoles = ['SUPER_USER'])
    {
        return Role::orderBy('id')->whereNotIn('type', $notChangeAbleRoles)->get();
    }

    public static function checkOrMakeRolesByUser($userId, $roleId)
    {
        return UserRoles::firstOrCreate([
            'user_id' => $userId,
            'role_id' => $roleId
        ]);
    }

    public static function removeRolesWithoutUs($userId, $rolesId = [])
    {
        return UserRoles::whereUserId($userId)->whereNotIn('role_id', $rolesId)->delete();
    }

    public static function versionOld()
    {
    ...
    public static function websiteTitleOld()
    {
    ...
    {
        return 'Jahangirnagar University';
    }

    public static function users($type = null, $data = null)
    {
        if ($type != null and $data != null) {
    ...
    public static function pieChartOld($title, $labels, $data, $colors = null)
    {
    ...
    public static function barChartOld($title, $element_label, $labels, $data, $colors = null)
    {
        $id = Laralum::randomString();
    ...
    public static function lineChartOld($title, $element_label, $labels, $data, $color = null)
    {
        $id = Laralum::randomString();
//        $default_color = Laralum::settings()->header_color;
        $default_color = '#1678c2';
...
//        $default_color = Laralum::settings()->header_color;
    ...
    public static function geoChartOld($title, $element_label, $data)
    {
        $id = Laralum::randomString();
...
//        return asset(Laralum::publicPath() . '/images/logo-text.png');
        return asset(Laralum::publicPath() . '/images/logo-ju.png');
    }
     ...
     * @return mixed
     */
    public static function imageResizeWithUpload($image, $path, $width, $height)
    {
        $image_resize = Image::make($image->getRealPath());
        $image_resize->resize($width, $height);
        return $image_resize->save(Storage::path($path));
    }
}
