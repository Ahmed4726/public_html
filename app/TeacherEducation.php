...
class TeacherEducation extends Model
{
    ...
    public static function allWithOptionalFilter ($search = false, $teacherID = false, $status = false, $paginate = false, $relation = false)
    {
        ...
        $education = (new TeacherEducation())->newQuery();

        if($teacherID) $education = $education->whereTeacherId($teacherID);

        if($status == 'inactive') $education = $education->whereStatus(0);
        elseif($status != false && $status) $education = $education->whereStatus($status);

        if($search) $education = $education->where(function ($query) use ($search){
            $query->orWhere("institute", "regexp", "$search")
                ->orWhere("description", "regexp", "$search")
                ->orWhere("period", "regexp", "$search");
        });

        if($relation) $education = $education->with($relation);
