...
{{--                            <li class="breadcrumb-item"><a href="#">ICT Cell</a></li>--}}
                            <li aria-current="page" class="breadcrumb-item active" v-if="currentType">@{{ currentType }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12 d-flex justify-content-center">
                    <form @submit.prevent="searchMemberList()" class="form-inline">
                            ...
                            <input type="text" id="keyword" v-model="search" class="form-control col-md-2 m-10" placeholder="Name...">
                            ...
                            <select id="departmentId" v-model="department_id" class="form-control col-md-3 m-10">
                                <option value="">-- Select Department --</option>
                                    <optgroup :label="faculty.name" v-for="faculty in faculties">
                                            <option :value="department.id" v-for="department in faculty.departments">@{{department.name}}</option>
                                    </optgroup>
                            </select>
            ...
            <div class="row mb-2">
                ...
                <div class="col-md-10">
                    <p class="text-right">Showing <span v-html="members.from"></span> - <span v-html="members.to"></span> of <span v-html="members.total"></span></p>
                </div>
                <div class="col-md-1"></div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-3" v-for="member in members.data">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <h4 class="card-title">
                                <a v-if="member.external_link" :href="member.external_link" target="_blank">@{{ member.name }}</a>
                                <span v-else>@{{ member.name }}</span>
                            </h4>
                            <p class="card-text">
                                <span v-if="member.department_name">@{{member.designation}},</span> <span v-else>@{{member.designation}}</span>
                                <span v-if="member.department_name">@{{ member.department_name }}</span>
                                <span v-if="member.status == 2 && member.status_info" class="btn btn-warning" role="button" style="font-size: 12px;padding: 5px 10px;">@{{ member.status_info.name }}</span>
                                <span v-if="member.status == 3 && member.status_info" class="btn btn-secondary" role="button" style="font-size: 12px;padding: 5px 10px;">@{{ member.status_info.name }}</span>
                                <span v-if="member.status == 4 && member.status_info" class="btn btn-info" role="button" style="font-size: 12px;padding: 5px 10px;">@{{ member.status_info.name }}</span>
                            </p>
