    ...
    <div class="column">
        <div class="ui very padded segment">
            @if($allow and Schema::hasColumn($name, 'id'))
                <a href="{{ route('Laralum::CRUD_create', ['table' => $name]) }}" class="ui blue button">{{ trans('laralum.create') }}</a><br>
            @else
                <a class="ui disabled blue button">{{ trans('laralum.create') }}</a><br>
            @endif
            <br>
                      ...
                      @foreach($rows as $row)
                          <tr>
                              ...
                              @if($allow_edit and \Schema::hasColumn($name, 'id'))
                                  <td>
                                      <a href="{{ route('Laralum::CRUD_edit', ['table' => $name, 'id' => $row->id]) }}" class="ui blue button">{{ trans('laralum.edit') }}</a>
                                  </td>
                              @else
                                  <td>
                                      <a class="ui disabled blue button">{{ trans('laralum.edit') }}</a>
                                  </td>
                              @endif
                              <?php
                              ...
                              @if($del)
                                  <td>
                                      <a href="{{ route('Laralum::CRUD_delete', ['table' => $name, 'id' => $row->id]) }}" class="ui blue button">{{ trans('laralum.delete') }}</a>
                                  </td>
                              @else
                                  <td>
                                      <a class="ui disabled blue button">{{ trans('laralum.delete') }}</a>
                                  </td>
                              @endif
                          </tr>
                      @endforeach
